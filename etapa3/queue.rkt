...
(define (queue-empty? q)
  (if (and (null? (queue-left q)) (null? (queue-right q)) (= 0 (queue-size-l q)) (= 0 (queue-size-r q)))
      #t
      #f
      ))
...
(define (dequeue q)
  ( if (and (null? (queue-left q)) (not(null? (queue-right q)))); left-null right-!null-> iau elementele din right 
        (struct-copy queue q [left (cdr (reverse(queue-right q)))]    ; left->right -pop 
                             [size-l (- (queue-size-r q) 1)]          ;size-l->size-r-1
                             [right null]
                             [size-r 0])
        (struct-copy queue q [left (cdr (queue-left q))]      ; doar scot un element din left 
...
(define (top q)
  (if (null? (queue-left q)) ; daca nu am in left
      (car (reverse(queue-right q))) ; iau din right 
      (car (queue-left q)) ; iau din left 
   ))

...
(define (queue-empty? q)
  (if (and (stream-empty? (queue-left q)) (null? (queue-right q)) (= 0 (queue-size-l q)) (= 0 (queue-size-r q)))
      #t
      #f
      ))
...
(define (rotate left right Acc)
  (if (stream-empty? left)
      (stream-cons (car right) Acc)  ;rotate([], [y], Acc)  ->   y : Acc
      (stream-cons (stream-first left) (rotate (stream-rest left) (cdr right) (stream-cons (car right) Acc) )) ;rotate((x:xs), (y:ys), Acc) = x : rotate(xs, ys, y : Acc)
   ))
...
(define (enqueue x q)
  (let(( new (struct-copy queue q [right (cons x (queue-right q))] [size-r (+ 1 (queue-size-r q))])))
    (if (= (queue-size-r new) (+ 1 (queue-size-l new)) )
        (struct-copy queue new [left (rotate (queue-left new) (queue-right new) empty-stream )]  [right null] [size-l (+ (queue-size-r new) (queue-size-l new))]  [size-r 0])
        new
     )
   )
  ) 
...
(define (dequeue q)
  (let (( new (struct-copy queue q [left (stream-rest (queue-left q))] [size-l (- (queue-size-l q) 1)] )))
    ( if (= (queue-size-r new) (+ 1 (queue-size-l new)) ) ; if rotation is needed
        (struct-copy queue new [left (rotate (queue-left new) (queue-right new) empty-stream )]  [right null] [size-l (+ (queue-size-r new) (queue-size-l new))]  [size-r 0])
        new
     )
    )
...
(define (top q)
    ( if (= (queue-size-r q) (+ 1 (queue-size-l q)) ) ; if rotation is needed
        (stream-first (queue-left (struct-copy queue q [left (rotate (queue-left q) (queue-right q) empty-stream )]
                                               [right null] [size-l (+ (queue-size-r q) (queue-size-l q))]  [size-r 0])))
